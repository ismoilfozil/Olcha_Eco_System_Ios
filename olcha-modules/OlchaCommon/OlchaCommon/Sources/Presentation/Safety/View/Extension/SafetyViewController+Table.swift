...
extension SafetyViewController: TableDelegates {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .biometric:
            break
        case .pincode:
            PincodeGlobalDefaults.session.isInitialPincode() ? coordinator?.pushAddPincode() : coordinator?.pushEditPincode()
        case .password:
...
extension SafetyViewController {
    public enum Section {
        ...
        public var title: String {
            switch self {
            case .pincode:
                return PincodeGlobalDefaults.session.isInitialPincode() ? "add_new_pincode".localized(.pincode) : "edit_pincode".localized(.pincode)
            case .password:
