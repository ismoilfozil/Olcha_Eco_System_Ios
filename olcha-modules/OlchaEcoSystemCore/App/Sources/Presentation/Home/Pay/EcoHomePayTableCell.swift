...
public class EcoHomePayTableCell: BaseTableCell {
    ...
    public override func configureViews() {
        ...
        collectionFlowDelegate.module = .pay
        ...
        headerView.setAllButton { [weak self] in
            ...
            observers?.clickActionSubject.send(PayClickAction.categories)
        }
    }
    
    public func setup(with model: BuilderSection) {
        collectionFlowDelegate.sectionModel = model
        headerView.setSectionLabel(model.section_name ?? "", image: .olchaPaySection)
