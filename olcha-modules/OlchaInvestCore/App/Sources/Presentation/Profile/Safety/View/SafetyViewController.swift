...
public class SafetyViewController: BaseViewController<BackNavigationBar> {
    ...
    public override func setupObservers() {
        handle(viewModel.$deleteUser, showLoader: true, withError: false, success: { [weak self] data in
    ...
    private func showDeleteSuccess() {
        showWarning(text: "delete_success_message".localized(.olchaInvestCore)) { [weak self] in
            guard let tabBarController = self?.tabBarController as? InvestMainTabBarController else {
                return
            }
            tabBarController.appCoordinator?.logout()
        }
    }
