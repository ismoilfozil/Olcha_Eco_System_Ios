...
public struct CardScanApi {
    ...
    // successfully
    nonisolated(unsafe) public static var lastScanStatsSuccess: Date?
    
    // XXX FIXME we should move to a more traditional error handling method
    public typealias ApiCompletion = @Sendable (_ response: [String: Any]?, _ error: ApiError?) -> Void
    
    nonisolated(unsafe) static public var baseUrl: String? = "https://api.getbouncer.com"
    ...
    nonisolated(unsafe) static public var apiKey: String?
        ...
        session.uploadTask(with: request, from: requestData) { data, response, error in
            ...
            DispatchQueue.main.async {
                if "ok" == responseData["status"] as? String || endpoint == "/v1/drivers_license/verify" {
                    CardScanApi.lastScanStatsSuccess = Date()
    ...
    @available(iOS 11.0, *)
    static public func downloadAndCompileLatestModel(signedUrl: String, completion: @escaping @Sendable (_ response: URL?, _ error: ApiError?) -> Void) {
    ...
    static public func getLatestModelConfig(modelClass: String, modelFrameworkVersion: String, parameters: [String: Any], completion: @escaping @Sendable (_ response: ModelConfigResponse?, _ error: ApiError?) -> Void) {
        if baseUrl == nil || apiKey == nil {
            DispatchQueue.main.async { completion(nil, apiUrlNotSet) }
            return
        }
