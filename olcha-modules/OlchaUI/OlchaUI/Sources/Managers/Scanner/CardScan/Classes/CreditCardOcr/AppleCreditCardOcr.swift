...
public class AppleCreditCardOcr: CreditCardOcrImplementation {
    public override func recognizeCard(in fullImage: CGImage, roiRectangle: CGRect) -> CreditCardOcrPrediction {
        ...
        AppleOcr.performOcr(image: image) { results in
            for result in results {
                let predictedPan = CreditCardOcrPrediction.pan(result.text)
                let expiry = CreditCardOcrPrediction.likelyExpiry(result.text)
                if let (month, year) = expiry, CreditCardUtils.isValidDate(expMonth: month, expYear: year) {
                    if expiryMonth == nil {
                ...
                if AppleCreditCardOcr.likelyName(result.text) != nil {
        ...
        let names = nameCandidates.filter { candidate in
            let isInExpectedLocation = minY.map({ candidate.rect.minY >= ($0 - 5.0) }) ?? false
            return candidate.confidence >= 0.5 && isInExpectedLocation
        }
