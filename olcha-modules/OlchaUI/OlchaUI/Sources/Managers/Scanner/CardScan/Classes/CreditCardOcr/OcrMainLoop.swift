...
@MainActor public protocol OcrMainLoopDelegate: AnyObject {
...
open class OcrMainLoop : MachineLearningLoop, @unchecked Sendable {
    ...
    func combine(prediction: CreditCardOcrPrediction) -> CreditCardOcrResult? {
        let shouldUse = MainActor.assumeIsolated {
            mainLoopDelegate?.shouldUsePrediction(errorCorrectedNumber: errorCorrection.number, prediction: prediction)
        } ?? true
        guard shouldUse else { return nil }
