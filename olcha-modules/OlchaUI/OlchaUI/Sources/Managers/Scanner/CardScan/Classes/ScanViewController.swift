...
//    @IBOutlet weak var backButtonImageToTextConstraint: NSLayoutConstraint!
    ...
    @IBOutlet weak var blurView: ScanBlurView!
...
//    @IBOutlet weak var backButton: UIButton!
    ...
    @IBOutlet weak var torchButtonImageView: UIImageView!
    ...
    var cornerBorderColor = UIColor.clear.cgColor//KapColors.appDarkColor.color.cgColor
    var denyPermissionTitle = "Need camera access".localized()
    var denyPermissionMessage = "Please enable camera access in your settings to scan your card".localized()
    var denyPermissionButtonText = "OK".localized()
    ...
    @objc static public func createViewController(withDelegate delegate: ScanDelegate? = nil) -> ScanViewController? {
        ...
        ScanViewController.configure(apiKey: "123321")
        CreditCardUtils.prefixesRegional = ["8600", "9860", "5614"]
        
        return self.createViewController(withDelegate: delegate, configuration: ScanConfiguration())
    }
    
    public override func updateLanguage() {
        self.setStrings()
    }
    ...
    @objc static public func createViewController(withDelegate delegate: ScanDelegate? = nil, configuration: ScanConfiguration) -> ScanViewController? {
        ...
        let storyboard = UIStoryboard(name: "CardScan", bundle: Bundle(identifier: BundleType.ui.identifier))
...
//        self.scanCardLabel.text = "Scan Card".localized() // dataSource.scanCard()
...
//        if let font = self.positionCardFont {
        self.positionCardLabel.font = UIFont.style(.medium, 14)
//        }
...
//        }
        torchButtonImageView.image = .flash?.scaleTo(CGSize(width: 26.0, height: 26.0))
...
//        self.present(alert, animated: true, completion: nil)
        let alert = UIAlertController(title: self.denyPermissionTitle, message: self.denyPermissionMessage, preferredStyle: .alert)
        alert.view.tintColor = .olchaAccentColor
        alert.addAction(UIAlertAction(title: "open_settings".localized(), style: .default) { action in
...
//        self.torchButton.setImage(UIImage(named: "flashlight-1")?.withRenderingMode(UIImage.RenderingMode.alwaysTemplate), for: UIControl.State.normal)
//        self.torchButton.tintColor = ColorConstants.appYellowColor
//        self.torchButton.backgroundColor = KapColors.viewBackgroundColor.color
//        self.torchButton.layer.cornerRadius = 16
//        self.positionCardLabel.font = CommonsFontType.medium.fontType(size: 18)
        self.startCameraPreview()
        
//        self.scanCardLabel.text = "sacn_card_title".localized()
        self.positionCardLabel.text = "scan_title".localized()
    }
