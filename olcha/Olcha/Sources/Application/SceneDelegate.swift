...
public class SceneDelegate: UIResponder, UIWindowSceneDelegate, SceneDelegateProtocol {
    ...
    public func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let notificationUserInfo = connectionOptions.notificationResponse?.notification.request.content.userInfo
        ...
        ModuleGenerator.shared.generate(module: .olcha, appStarted: { [weak self] in
            self?.routeNotification(userInfo: notificationUserInfo)
        })
...
//        (UIApplication.shared.delegate as? AppDelegate)?.saveContext()
    ...
    private func routeNotification(userInfo: [AnyHashable: Any]?) {
        ...
        if let action = Funcs.clickActionMapper(jsonParams: userInfo)?.getAction() {
            ...
            getAppCoordinator()?.notificationRouter(notification: Funcs.jsonMapper(jsonParams: userInfo))
        }
    }
